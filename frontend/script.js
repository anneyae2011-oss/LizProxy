 ...
 * Calls generate-key endpoint which returns full key for existing users
 */
async function checkExistingKey() {
    try {
        ...
        // Call generate-key endpoint - it returns full key for existing users
...
async function generateKey() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log('generateKey API response:', JSON.stringify(data));
            console.log('generateKey - data.key:', data.key);
            console.log('generateKey - data.key type:', typeof data.key);
            console.log('generateKey - data.key length:', data.key ? data.key.length : 'null');
            
            // If this is a new key, we get the full key
            if (data.key) {
                fullKey = data.key;
                localStorage.setItem(STORAGE_FULL_KEY, data.key);
                console.log('generateKey - Stored full key in localStorage');
                console.log('generateKey - Verify localStorage:', localStorage.getItem(STORAGE_FULL_KEY));
                showFullKey(data.key);
            } else {
                console.log('generateKey - No full key returned (existing key for IP)');
            }
...
async function copyKey() {
    ...
    // Fallback: try to get from the displayed text element
    if (!keyToCopy && fullKeyText && fullKeyText.textContent && fullKeyText.textContent.startsWith('sk-')) {
        keyToCopy = fullKeyText.textContent;
    }
    
    // Debug: log what we're trying to copy
    console.log('=== COPY KEY DEBUG ===');
    console.log('localStorage STORAGE_FULL_KEY:', localStorage.getItem(STORAGE_FULL_KEY));
    console.log('In-memory fullKey:', fullKey);
    console.log('fullKeyText.textContent:', fullKeyText ? fullKeyText.textContent : 'element not found');
    console.log('Final keyToCopy:', keyToCopy);
    console.log('keyToCopy length:', keyToCopy ? keyToCopy.length : 'null');
...
/**
 * Show the full key (always visible now since it's IP-based)
 */
function showFullKey(key) {
    fullKeyText.textContent = key;
    fullKeyDisplay.classList.remove('hidden');
    
    // Also store in memory and localStorage again to be safe
    fullKey = key;
    localStorage.setItem(STORAGE_FULL_KEY, key);
    
    // Make the key text selectable for manual copy
    fullKeyText.style.userSelect = 'text';
    fullKeyText.style.cursor = 'text';
    
    // Key stays visible - no auto-hide since it's IP-based (user requested)
}

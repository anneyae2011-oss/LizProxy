...
def create_database(database_url: Optional[str] = None, database_path: str = "./proxy.db") -> "Database":
    ...
    Args:
        database_url: PostgreSQL connection URL (if provided, uses PostgreSQL)
        database_path: SQLite file path (used if database_url is None)
    
    Returns:
        Database instance (either SQLite or PostgreSQL)
    """
    if database_url:
        if not HAS_ASYNCPG:
...
class Database(ABC):
    ...
    # API Key operations
    @abstractmethod
    async def create_api_key(self, ip_address: str, key_hash: str, key_prefix: str, full_key: str, browser_fingerprint: Optional[str] = None) -> int:
        pass
...
class SQLiteDatabase(Database):
    ...
    async def initialize(self) -> None:
        ...
        # Migration: Add browser_fingerprint if missing
        try:
            await conn.execute("SELECT browser_fingerprint FROM api_keys LIMIT 1")
        except Exception:
            await conn.execute("ALTER TABLE api_keys ADD COLUMN browser_fingerprint TEXT")
        
        # Migration: Add full_key if missing
        try:
            await conn.execute("SELECT full_key FROM api_keys LIMIT 1")
        except Exception:
    ...
    async def create_api_key(self, ip_address: str, key_hash: str, key_prefix: str, full_key: str, browser_fingerprint: Optional[str] = None) -> int:
        ...
        cursor = await conn.execute(
            "INSERT INTO api_keys (ip_address, key_hash, key_prefix, full_key, browser_fingerprint) VALUES (?, ?, ?, ?, ?)",
            (ip_address, key_hash, key_prefix, full_key, browser_fingerprint)
        )
        await conn.commit()
    ...
    async def get_key_analytics(self, key_id: int) -> Optional[KeyAnalytics]:
        conn = await self._get_connection()
        cursor = await conn.execute("SELECT key_prefix, ip_address FROM api_keys WHERE id = ?", (key_id,))
...
class PostgreSQLDatabase(Database):
    ...
    async def initialize(self) -> None:
        ...
        async with pool.acquire() as conn:
            ...
            # Migration: Add full_key column if missing
            try:
                await conn.execute("ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS full_key TEXT")
            except Exception:
    ...
    async def create_api_key(self, ip_address: str, key_hash: str, key_prefix: str, full_key: str, browser_fingerprint: Optional[str] = None) -> int:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "INSERT INTO api_keys (ip_address, key_hash, key_prefix, full_key, browser_fingerprint) VALUES ($1, $2, $3, $4, $5) RETURNING id",
                ip_address, key_hash, key_prefix, full_key, browser_fingerprint
            )
            return row["id"]
    ...
    async def get_key_analytics(self, key_id: int) -> Optional[KeyAnalytics]:
        pool = await self._get_pool()
        async with pool.acquire() as conn:
            key_row = await conn.fetchrow("SELECT key_prefix, ip_address FROM api_keys WHERE id = $1", key_id)
            if not key_row:
                return None

...
class Database(ABC):
    ...
    # API Key operations
    @abstractmethod
    async def create_api_key(self, ip_address: str, key_hash: str, key_prefix: str, browser_fingerprint: Optional[str] = None) -> int:
        pass
...
class SQLiteDatabase(Database):
    ...
    async def create_api_key(self, ip_address: str, key_hash: str, key_prefix: str, browser_fingerprint: Optional[str] = None) -> int:
        ...
        cursor = await conn.execute(
            "INSERT INTO api_keys (ip_address, key_hash, key_prefix, browser_fingerprint) VALUES (?, ?, ?, ?)",
            (ip_address, key_hash, key_prefix, browser_fingerprint)
        )
        await conn.commit()
    ...
    def _row_to_api_key(self, row) -> ApiKeyRecord:
        return ApiKeyRecord(
            id=row["id"], key_hash=row["key_hash"], key_prefix=row["key_prefix"], ip_address=row["ip_address"],
    ...
    async def create_api_key(self, ip_address: str, key_hash: str, key_prefix: str, browser_fingerprint: Optional[str] = None) -> int:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "INSERT INTO api_keys (ip_address, key_hash, key_prefix, browser_fingerprint) VALUES ($1, $2, $3, $4) RETURNING id",
                ip_address, key_hash, key_prefix, browser_fingerprint
            )
            return row["id"]

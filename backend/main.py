...
async def verify_admin_password(
    ...
    if settings and hmac.compare_digest(x_admin_password, settings.admin_password):
...
async def generate_key_endpoint(
    ...
    Key lookup priority:
    1. Same IP → return existing key (regardless of browser)
    2. Same fingerprint, different IP → update IP and return existing key
    3. New IP + new fingerprint → generate new key
    
    Returns:
        KeyGenerationResponse with the key (only on first generation) and prefix.
    """
    ...
    if existing_key:
        ...
        return KeyGenerationResponse(
            ...
            message="An API key already exists for your IP address. The full key was shown only once during initial generation."
        )
    ...
    if fingerprint:
        ...
        if fingerprint_key:
            ...
            return KeyGenerationResponse(
                ...
                message="Welcome back! Your IP changed but we recognized your browser. Your existing key is still valid."
            )
    ...
    # Store in database with fingerprint
